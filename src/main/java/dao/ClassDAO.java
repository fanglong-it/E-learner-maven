...
public class ClassDAO {
	...
	public List<model.Class> getClassByTeacherId(int teacher) throws SQLException, Exception {
		...
		try {
			...
			if (con != null) {
				...
				rs = ps.executeQuery();
				while (rs.next()) {
					classes.add(model.Class.builder().id(rs.getInt("Id")).className(rs.getString("className"))
	...
	public model.Class getClassById(int classId) throws SQLException, Exception {
		...
		try {
			...
			if (con != null) {
				...
				rs = ps.executeQuery();
				if (rs.next()) {
					return model.Class.builder().id(rs.getInt("Id")).className(rs.getString("className"))
							.maxStudent(rs.getInt("maxStudent")).account(accountDAO.getAccountById(rs.getInt("userId")))
							.dateCreate(rs.getDate("dateCreate")).image(rs.getString("image"))
	...
	public List<model.Class> getClassByCourseId(int courseId) throws SQLException, Exception {

...
public class AccountDAO implements Serializable {
	...
	public Account checkLogin(String user, String pass) throws SQLException, Exception {
		...
		try {
			...
			if (con != null) {
				...
				rs = ps.executeQuery();
				if (rs.next()) {
					return Account.builder().id(rs.getInt("id")).username(rs.getString("username"))
	...
	public Account updateAccount(Account account) throws SQLException, Exception {
		String query = "UPDATE Account" + " SET username=?, status=?, email=?, phone=?,"
				+ " fullname=?, address=?, avatar=?, roleId=?" + " WHERE id=?;";
		try {
			con = DBContext.makeConnection();
			if (con != null) {
				ps = con.prepareStatement(query);

...
public class MemberDAO {
	...
	public Member getLastMemberChat() throws SQLException, Exception {
		...
		try {
			...
			if (con != null) {
				...
				rs = ps.executeQuery();
				if (rs.next()) {
					return Member.builder().id(rs.getInt("id"))
							.groupChat(groupChatDAO.getGroupChatById(rs.getInt("groupChatId")))
							.account(accountDAO.getAccountById(rs.getInt("userId"))).build();
				}
			}
		} finally {
			if (rs != null) {
	...
	public Member saveMemberChat(Member member) throws SQLException, Exception {
		String query = "INSERT INTO [Member] (groupChatId, userId, isLeader) VALUES(?, ?,?)";
		try {
			con = DBContext.makeConnection();
			if (con != null) {
				ps = con.prepareStatement(query);
				...
				ps.setInt(3, member.getIsLeader());
